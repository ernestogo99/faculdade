 ...
 void enfilar(double elem)
{
    ...
    else
    {
        if ((prim == 0 and ult == tam - 1) or (ult + 1 == prim))
        {
            ...
            for (int idx = prim; idx != ult; avancar(idx))
            {
                w[i] = v[idx];
                i++;
            }
            w[i] = v[ult]; // Copie o último elemento também.
    ...
    double ultimo()
    {
        if (vazia())
        {
            throw errofila("consulta do ultimo em fila vazia");
        }
        return v[ult];
    }
...
int main()
{
    try
    {
        ...
        double first = f.primeiro();
        cout << "o primeiro da fila e: " << first<<'\n';
        double last=f.ultimo();
        cout<<"o ultimo da fila e: "<<last<<'\n';
        int tam = f.tamanho();
        cout << "o tamanho da fila e: " << tam<<'\n';
