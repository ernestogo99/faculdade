...
int main(int narg, char *argv[]) {
    char num[3], numextenso_array[1000][1024];
    FILE *arq;
    ...
    if (narg == 1) {
        ...
        valor = atoi(num);
        if (valor >= 100)
            continua = converte_centenas(&valor, i, ext, numextenso_array);
        if (valor < 100 && valor >= 10)
            continua = converte_dezenas(&valor, i, ext, numextenso_array);
        if (valor <= 9 && valor != 0 && continua != 0)
            converte_unidades(&valor, i, ext, numextenso_array);
    } else
        arq = fopen(argv[1], "r");
